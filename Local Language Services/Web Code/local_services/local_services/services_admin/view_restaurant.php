<?php
	...
	if(isset($_GET['status']) && isset($_GET['id'])) {
		$arr = array(
			'restaurant_status' => $_GET['status']
		);
		$condition = " restaurant_id = '".$_GET['id']."' ";
	...
	if(isset($_POST['command']) && $_POST['command'] == "m delete") {
		...
		$condition = " restaurant_id IN ($ids) ";
	...
	$where ="";
        ...
        echo $where .= "AND restaurant_name like '%".$_POST['restaurant_name']."%'";
        }
      ...
      <header id="topbar">
        <div class="topbar-left">
          <ol class="breadcrumb">
            ...
            <li class="crumb-trail">View Restaurant</li>
          </ol>
        </div>
      ...
      <!--      <label for="topbar-multiple" class="control-label pr10 fs11 text-muted" style="color:#555;";><?php echo date('l jS \of F Y , h:i:s A'); ?></label>   -->
		...
		<section id="content" class="table-layout animated fadeIn">
		   <div class="form-group " style="padding-bottom: 0;">
		   	  ...
		   	  <form class="" id="" role="form" method="post" action="home.php?pages=view-restaurant" >
		    	<div class="col-md-3 plp1 ">
		     	  <input type="text" id="restaurant_name" class="form-control product" value="<?=$_REQUEST['restaurant_name']?>" name="restaurant_name" autocomplete="off" placeholder="Search By Restaurant Name">
		    	</div>
				...
				<div class="panel panel-warning panel-border top mb35">
				  <div class="panel">
					...
					<form role="form" name="form1" method="post" enctype="multipart/form-data">
						...
						<table class="table admin-form theme-warning tc-checkbox-1 fs13">
						  <thead>
							<tr class="bg-light">
							  ...
							  <th class="" style="width: 200px;">Address</th>
						  ...
						  <tbody>
							<?php
							$ex_data = $db->db_get_array("SELECT * FROM $table WHERE 1=1 $where ORDER BY restaurant_id DESC LIMIT $start, $end");
							$tot = count($ex_data);
							if ( $tot > 0) {
							...
							<tr>
							  <td class="">
								<label class="option block mn" style="width: 55px;">
								  <input type="checkbox" name="chk[]" value="<?=$data['restaurant_id']?>" onClick="uncheck()" >
								  <span class="checkbox mn"></span>
								</label>
							  </td>

							  <td class="">
								<a title="Edit" href="home.php?pages=add-restaurant&edit_id=<?=$data['restaurant_id']?>" style="text-transform: capitalize;"><?=$data['restaurant_name']?></a>
							  </td>
							 ...
							 ?>
							  ...
							  <td class="" >
								...
								?>
								<img src="../uploads/<?=$data['restaurant_image']?>" width="100"/>
								<?php
								} else {
								?>
								- - - - -
								...
								?>
                                                             ...
                                                             <td class="" >
								<?php
								if($data['restaurant_language'] != "") {
								?>
								<?=$data['restaurant_language']?>
								<?php
								} else {
								?>
								- - - - -
							  ...
							  <td class="" >
								<?php
								if($data['restaurant_created'] != "") {
								?>
								<?= date('d F Y',strtotime($data['restaurant_created']))?>
								<?php
								} else {
								?>
								- - - - -
								...
								<?php
								if($data['restaurant_status'] != 0) {
								?>

								<td class="text-right">
									<a href="home.php?pages=view-restaurant&status=0&id=<?=$data['restaurant_id']?>" class="" title="Show">
								...
								?>
								<td class="text-right">
								<a href="home.php?pages=view-restaurant&status=1&id=<?=$data['restaurant_id']?>" class="" title="Hide">
					  ...
					  <?php
						$ex_data = $db->db_get_array("SELECT * FROM $table WHERE 1=1 $where ORDER BY restaurant_id DESC ");
						$new = count($ex_data);
					  ...
					  ?>
					    <div class="dt-panelfooter clearfix">
							...
							<div class="dataTables_paginate paging_simple_numbers" id="datatable_paginate">
								...
								$prev_link = ($_GET['start'] > 0) ? "home.php?pages=view-restaurant&start=".($_GET['start'] - 1) : "javascript: return void(0)";
								?>
								...
								for($pn = 0; $pn < $tot_page; $pn++) {
									$page_link = ($_GET['start'] == $pn) ? "javascript: return void(0)" : "home.php?pages=view-restaurant&start=".$pn;
								?>
								...
								$next_link = ($_GET['start'] < $tot_page - 1) ? "home.php?pages=view-restaurant&start=".($_GET['start'] + 1) : "javascript: return void(0)";
								?>
