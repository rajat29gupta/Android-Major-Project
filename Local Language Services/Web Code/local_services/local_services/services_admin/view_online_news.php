<?php
	...
	if(isset($_GET['status']) && isset($_GET['id'])) {
		$arr = array(
			'online_news_status' => $_GET['status']
		);
		$condition = " online_news_id = '".$_GET['id']."' ";
	...
	if(isset($_POST['command']) && $_POST['command'] == "m delete") {
		...
		$condition = " online_news_id IN ($ids) ";
	...
	$where ="";
        if($_POST['online_news_title']!=""){
        echo $where .= "AND online_news_title like '%".$_POST['online_news_title']."%'";
        }
      ...
      <header id="topbar">
        <div class="topbar-left">
          <ol class="breadcrumb">
            ...
            <li class="crumb-trail">View Online News</li>
          </ol>
        </div>
      ...
      <!--      <label for="topbar-multiple" class="control-label pr10 fs11 text-muted" style="color:#555;";><?php echo date('l jS \of F Y , h:i:s A'); ?></label>   -->
		...
		<section id="content" class="table-layout animated fadeIn">
		   <div class="form-group " style="padding-bottom: 0;">
		   	  ...
		   	  <form class="" id="" role="form" method="post" action="home.php?pages=view-online-news" >
		    	<div class="col-md-3 plp1 ">
		     	  <input type="text" id="online_news_title" class="form-control product" value="<?=$_REQUEST['online_news_title']?>" name="online_news_title" autocomplete="off" placeholder="Search By Online News Title">
		    	</div>
					...
					<form role="form" name="form1" method="post" enctype="multipart/form-data">
					  ...
					  <div class="table-responsive">
						<table class="table admin-form theme-warning tc-checkbox-1 fs13">
						  ...
						  <tbody>
							<?php
							$ex_data = $db->db_get_array("SELECT * FROM $table WHERE 1=1 $where ORDER BY online_news_id DESC LIMIT $start, $end");
							$tot = count($ex_data);
							if ( $tot > 0) {
							...
							<tr>
							  <td class="">
								<label class="option block mn" style="width: 55px;">
								  <input type="checkbox" name="chk[]" value="<?=$data['online_news_id']?>" onClick="uncheck()" >
								  <span class="checkbox mn"></span>
								</label>
							  </td>

							  <td class="">
								<a title="Edit" href="home.php?pages=add-online-news&edit_id=<?=$data['online_news_id']?>" style="text-transform: capitalize;"><?=$data['online_news_title']?></a>
							  </td>

							  <td class="" >
								<?php
								if($data['online_news_description'] != "") {
								?>
								<?=$data['online_news_description']?>
								<?php
								} else {
								?>
								- - - - -
							  ...
							  <td class="" >
								<?php
								if($data['online_news_image'] != "") {
								?>
								<img src="../uploads/<?=$data['online_news_image']?>" width="100"/>
								<?php
								} else {
								?>
								- - - - -
								...
								?>
                                                             ...
                                                             <td class="" >
								<?php
								if($data['online_news_language'] != "") {
								?>
								<?=$data['online_news_language']?>
								<?php
								} else {
								?>
								- - - - -
							  ...
							  <td class="" >
								<?php
								if($data['online_news_created'] != "") {
								?>
								<?= date('d F Y',strtotime($data['online_news_created']))?>
								<?php
								} else {
								?>
								- - - - -
								...
								<?php
								if($data['online_news_status'] != 0) {
								?>

								<td class="text-right">
									<a href="home.php?pages=view-online-news&status=0&id=<?=$data['online_news_id']?>" class="" title="Show">
								...
								?>
								<td class="text-right">
								<a href="home.php?pages=view-online-news&status=1&id=<?=$data['online_news_id']?>" class="" title="Hide">
					  ...
					  <?php
						$ex_data = $db->db_get_array("SELECT * FROM $table WHERE 1=1 $where ORDER BY online_news_id DESC ");
						$new = count($ex_data);
					  ...
					  ?>
					    <div class="dt-panelfooter clearfix">
							...
							<div class="dataTables_paginate paging_simple_numbers" id="datatable_paginate">
								...
								$prev_link = ($_GET['start'] > 0) ? "home.php?pages=view-online-news&start=".($_GET['start'] - 1) : "javascript: return void(0)";
								?>
								...
								for($pn = 0; $pn < $tot_page; $pn++) {
									$page_link = ($_GET['start'] == $pn) ? "javascript: return void(0)" : "home.php?pages=view-online-news&start=".$pn;
								?>
								...
								$next_link = ($_GET['start'] < $tot_page - 1) ? "home.php?pages=view-online-news&start=".($_GET['start'] + 1) : "javascript: return void(0)";
								?>
